...
export const CountriesContextProvider = ({ children }) => {
  ...
  const filteredCountry = getResults(allCountries, query);
  console.log(filteredCountry);

  const randomCountries = useMemo(() => {
    const randomCountry = allCountries.filter((country) => {
      if (query) return getResults(allCountries, query);
      if (!query) {
        if (search === "All") {
  ...
  return (
    <countriesContext.Provider value={[randomCountries, filteredCountry]}>
